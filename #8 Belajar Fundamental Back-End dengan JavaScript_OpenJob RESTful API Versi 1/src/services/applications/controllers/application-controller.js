...
export const create = async (req, res, next) => {
  ...
  return response(res, 201, "Application berhasil ditambahkan", { id: application.id });
};
...
export const findById = async (req, res, next) => {
  ...
  return response(res, 200, "Application berhasil ditampilkan", { id: application.id });
};

export const findByUser = async (req, res, next) => {
  const { id } = req.params;
  const applications = await applicationRepository.findByUser(id);
  if (!applications) {
    return next(new NotFoundError("Application tidak ditemukan"));
  }
...
export const findByJob = async (req, res, next) => {
  const { id } = req.params;
  const applications = await applicationRepository.findByJob(id);
  if (!applications) {
    return next(new NotFoundError("Application tidak ditemukan"));
  }
  return response(res, 200, "Application berhasil ditampilkan", { applications });
...
export const update = async (req, res, next) => {
  ...
  return response(res, 200, "Application berhasil diperbarui", { id: application.id });
};
...
export const destroy = async (req, res, next) => {
  ...
  return response(res, 200, "Application berhasil dihapus", { id: application.id });
};
